...
The bonus task required a custom domain registered in Route53. I do not currently have a Route53 domain, so I did not complete the Route53/ACM portion. Instead, the website is served over HTTPS via the **CloudFront default certificate** on a `*do19ozu6426z2.cloudfront.net` domain, which is exactly what the main task requires and what the submission asks for in the *Live App Link* field.
